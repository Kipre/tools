...
bro.test("deletes unnecessary points", () => {
  ...
  path.lineTo([265, 60]);
  path.lineTo([305, 60]);
  path.lineTo([305, -32.7755905511811]);
  path.lineTo([340, -50]);
  path.lineTo([340, -100]);
  path.lineTo([207, -100]);
  path.lineTo([207, 0]);
...
bro.test("drag drags", () => {
  ...
  const circle = Path.makeCircle(10).translate([-50, 0]);
  bro
    .expect(circle.drag([100, 0]).toString())
    .toEqual(
      "M -50 -10 A 10 10 0 0 0 -60 0 A 10 10 0 0 0 -50 10 L 50 10 A 10 10 0 0 0 60 0 A 10 10 0 0 0 50 -10 Z",
    );
});
